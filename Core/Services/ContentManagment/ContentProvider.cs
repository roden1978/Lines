...
public interface IContentProvider : IStartable
{
    ...
    int TilesCount {get;}
    int GetRandomTileIndex(bool all = false);
    int GetRandomIndex();
...
public class ContentProvider : IContentProvider
{
    ...
    public Sprite GetTileSpriteByIndex(int index) => _tiles.GetSpriteByIndex(index);

    public int GetRandomTileIndex(bool all = false)
    {
        int startIndex = false == all ? 1 : 0;
        return _random.Next(startIndex, TilesCount);
    }

    public int GetRandomIndex() => _random.Next(0, Settings.TilesCount * Settings.TilesCount);

    public void Start() => LoadAll();

    public Sprite CreateDefaultSprite() => _contentLoadService.CreateDefaultSprite();
}
